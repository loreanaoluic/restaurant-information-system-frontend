...
export class StepperOrderComponent {
  ...
  constructor(private waiterService: WaiterService, private modalService: MdbModalService) {}

  getReceiptItems() {
    this.waiterService.getAllReceiptItems().subscribe(
      (response) => {
        this.receiptItems = response;
        this.totalPrice = 0;
        for (let i = 0; i < this.receiptItems.length; i++) {
          this.totalPrice += this.receiptItems[i].item.price.value * this.receiptItems[i].quantity;
        }
    });
  }

  openModal(receiptItem: ReceiptItem) {
    this.modalRef = this.modalService.open(NoteModalComponent, { data: { receiptItem: receiptItem }
  });
  }
